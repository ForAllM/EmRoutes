...
const SelectButton = ({title,handlePress,containerStyle,textStyle,isLoading,options,optionStyle}) => {
  ...
  return (
    <View className={`space-y-2 ${containerStyle}`}>
      ...
      <TouchableOpacity
        className={`border-2 border-primary flex-row rounded-2xl w-full h-16 justify-between px-4 bg-white focus:border-secondary items-center`}
        onPress={() => setIsOpen(!isOpen)}
      >
        <Text className={`text-red-700 font-psemibold text-lg ${textStyle}`}>{selectOption || "Select an Option"}</Text>
        <Icon
